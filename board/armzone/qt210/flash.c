...
#define MEM_FLASH_RESET		(*(volatile u16 *)(CONFIG_SYS_FLASH_BASE))
...
int flash_erase_all (flash_info_t * info)
{
	...
	while(1) {
		...
		udelay_masked (10000);
		if (readl(CONFIG_SYS_FLASH_BASE) == 0xffffffff)
			break;
		printf(".");
		}
	printf("\nERASE ALL is DONE\n");
...
{
	volatile u32 *buf = CONFIG_PHY_UBOOT_BASE;
	volatile u32 *nor_base = CONFIG_SYS_FLASH_BASE;
	
	memcpy ((void *)buf, (void *)nor_base, PART_SIZE_BL);	
}
