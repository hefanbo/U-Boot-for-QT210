...
void movi_zImage_copy(void)
{
	 ...
	 */
	ret =
	    copy_zImage(0, 0x3C6FCE, MOVI_ZIMAGE_BLKCNT, CONFIG_PHY_KERNEL_BASE,
			1);
...
void movi_write(ulong start, lbaint_t blkcnt, void *src)
{
	struct mmc *m = find_mmc_device(0);
	m->block_dev.block_write(0, start, blkcnt, src);
}

void movi_read(ulong start, lbaint_t blkcnt, void *dst)
{
	struct mmc *m = find_mmc_device(0);
	m->block_dev.block_read(0, start, blkcnt, dst);
}
