...
#if defined(CONFIG_ENV_IS_IN_AUTO) /* Environment is in Non-Volatile Device */
...
int saveenv_nand(void)
{
        ...
        /* If the value of CONFIG_ENV_OFFSET is not a NAND block boundary, the
         * NAND erase operation will fail. So first check if the CONFIG_ENV_OFFSET
         * is equal to a NAND block boundary
         */
        if ((CONFIG_ENV_OFFSET % (erasesize - 1)) != 0 ) {
                /* CONFIG_ENV_OFFSET is not equal to block boundary address. So, read
                ...
                /* CONFIG_ENV_OFFSET is equal to a NAND block boundary. So
...
int saveenv_onenand(void)
{
	...
	/* If the value of CONFIG_ENV_OFFSET is not a OneNAND block boundary, the
	 * OneNAND erase operation will fail. So first check if the CONFIG_ENV_OFFSET
	 * is equal to a NAND block boundary
	 */
	if ((CONFIG_ENV_OFFSET % (erasesize - 1)) != 0 ) {
		/* CONFIG_ENV_OFFSET is not equal to block boundary address.
		 ...
		 */
		erasebase = (CONFIG_ENV_OFFSET / erasesize) * erasesize;
		data = (uint8_t*)malloc(erasesize);
		if (data == NULL) {
			printf("Could not save enviroment variables\n");
			return 1;
		...
		/* CONFIG_ENV_OFFSET is equal to a OneNAND block boundary. So
		 * no special care is required when erasing and writing OneNAND
		 * block
		 */
		data = env_ptr;
		erasebase = CONFIG_ENV_OFFSET;
		writesize = CONFIG_ENV_SIZE;
	}
...
int saveenv_nor(void)
{
	...
	pos_env = CONFIG_SYS_FLASH_BASE + CONFIG_ENV_ADDR + CONFIG_ENV_OFFSET;
	...
	memcpy(tmp_pos_env, (u8 *)env_ptr, CONFIG_ENV_SIZE);
...
void env_relocate_spec_nor(void)
{
	...
	env_src = (u_char *)(CONFIG_SYS_FLASH_BASE + CONFIG_ENV_OFFSET);
	env_dst = (u_char *)malloc(CONFIG_ENV_SIZE);
	total = CONFIG_ENV_SIZE;
...
void env_relocate_spec_onenand(void)
{
	...
	ret = onenand->read(onenand, CONFIG_ENV_OFFSET, CONFIG_ENV_SIZE, &total, (u_char*)env_ptr);
	if (ret || total != CONFIG_ENV_SIZE)
		return use_default();
...
{
printf("HEFB: entering env_relocate_spec()\n");
printf("HEFB: INF_REG3 = %d\n", INF_REG3_REG);
printf("HEFB: fixme! Store boot mode in INF_REG3 at the beginning\n");
printf("HEFB: bypassing INF_REG3 check\n");
env_relocate_spec_movinand(); // TODO: re-enable following codes
/*
#if defined(CONFIG_SMDKC100) | defined(CONFIG_SMDKC110) | defined(CONFIG_S5P6442) | defined(CONFIG_QT210)
	if (INF_REG3_REG == 1)
